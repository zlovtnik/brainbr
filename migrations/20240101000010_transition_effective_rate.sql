...
SELECT
    year,
    CASE
        WHEN year < 2026 THEN 0.0
        WHEN year = 2026 THEN 0.10
        WHEN year = 2027 THEN 0.25
        WHEN year = 2028 THEN 0.40
        WHEN year = 2029 THEN 0.55
        WHEN year = 2030 THEN 0.70
        WHEN year = 2031 THEN 0.85
        ELSE 1.0
    END AS reform_weight,
    CASE
        WHEN year < 2026 THEN 1.0
        WHEN year = 2026 THEN 0.90
        WHEN year = 2027 THEN 0.75
        WHEN year = 2028 THEN 0.60
        WHEN year = 2029 THEN 0.45
        WHEN year = 2030 THEN 0.30
        WHEN year = 2031 THEN 0.15
        ELSE 0.0
    END AS legacy_weight
...
WITH calc AS (
    SELECT
        ...
        COALESCE((SELECT SUM((value)::numeric)
                  ...
                  WHERE value ~ '^-?([0-9]+\.?[0-9]*|[0-9]*\.[0-9]+)$'), 0) AS legacy_burden,
        COALESCE((SELECT SUM((value)::numeric)
                  FROM jsonb_each_text(i.reform_taxes)
                  WHERE value ~ '^-?([0-9]+\.?[0-9]*|[0-9]*\.[0-9]+)$'), 0) AS reform_burden,
