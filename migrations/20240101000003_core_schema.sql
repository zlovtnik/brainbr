...
DROP INDEX IF EXISTS uq_knowledge_law_ref_company;
CREATE UNIQUE INDEX IF NOT EXISTS uq_knowledge_law_ref_company
    ON fiscal_knowledge_base (law_ref, company_id) WHERE company_id IS NOT NULL;

CREATE UNIQUE INDEX IF NOT EXISTS uq_knowledge_law_ref_global
    ON fiscal_knowledge_base (law_ref) WHERE company_id IS NULL;
