...
BEGIN
	FOREACH _table IN ARRAY policy_tables LOOP
		EXECUTE format(
			'CREATE POLICY %I_tenant_policy ON %I USING (CASE WHEN current_setting(''app.bypass_rls'', true) = ''true'' THEN true ELSE company_id = current_setting(''app.current_company_id'', true)::uuid END) WITH CHECK (CASE WHEN current_setting(''app.bypass_rls'', true) = ''true'' THEN true ELSE company_id = current_setting(''app.current_company_id'', true)::uuid END);',
			_table,
			_table
		);
	END LOOP;
